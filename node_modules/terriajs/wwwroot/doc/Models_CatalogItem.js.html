...
var CatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the geographic rectangle (extent or bounding box) containing this data item.  This property is
     * observable.
     * @type {Rectangle}
     */
    ...
    /**
     * Gets or sets a message to show when this item is enabled for the first time in order to call attention to the
     * Now Viewing panel.
     * @type {String}
     */
    ...
    /**
     * The maximum number of features whose information can be shown at one time in the FeatureInfoPanelViewModel, from
     * this item. Defaults to terria.configParameters.defaultMaximumShownFeatureInfos
     * @type {Number}
     */
    ...
    knockout.track(this, ['rectangle', 'dataCustodian', 'attribution',
    ...
    knockout.defineProperty(this, 'legendUrls', {
        ...
        set: function(legendUrls) {
            this._legendUrls = legendUrls;
        }
    });
...
defineProperties(CatalogItem.prototype, {
    ...
    hasLegend: {
        get: function() {
            return defined(this.legendUrls) &amp;&amp; this.legendUrls.length;
        }
    },
     ...
     * CatalogMember#serializeToJson} is called and the `serializeForSharing` flag is set in the options.
     * @memberOf CatalogItem.prototype
     * @type {String[]}
     */
...
/**
 * Gets or sets the set of default updater functions to use in {@link CatalogMember#updateFromJson}.  Types derived
 * from this type should expose this instance - cloned and modified if necesary - through their {@link
 * CatalogMember#updaters} property.
 * @type {Object}
 */
...
CatalogItem.defaultUpdaters.legendUrl = function(catalogItem, json, prototypeName) {
    if (json.legendUrl) {
        catalogItem.legendUrls = [new LegendUrl(json.legendUrl)];
    }
};

CatalogItem.defaultUpdaters.legendUrls = function(catalogItem, json, prototypeName) {
    if (defined(json.legendUrls)) {
        catalogItem.legendUrls = json.legendUrls &amp;&amp; json.legendUrls.map(function(rawLegend) {
            return new LegendUrl(rawLegend.url, rawLegend._mimeType);
        });
    }
};
...
/**
 * Gets or sets the set of default serializer functions to use in {@link CatalogMember#serializeToJson}.  Types derived
 * from this type should expose this instance - cloned and modified if necesary - through their {@link
 * CatalogMember#serializers} property.
 * @type {Object}
 */
 ...
 * Derived classes may optionally implement {@link CatalogItem#_getValuesThatInfluenceLoad} to provide an array
 * containing the current value of all properties that influence this item's load process.  Each time that {@link
 * CatalogItem#load} is invoked, these values are checked against the list of values returned last time, and {@link
 * CatalogItem#_load} is invoked again if they are different.  If {@link CatalogItem#_getValuesThatInfluenceLoad} is
 * undefined or returns an empty array, {@link CatalogItem#_load} will only be invoked once, no matter how many times
...
/**
 * Moves the camera so that the data item's bounding rectangle is visible, and updates the TerriaJS clock according to
 * this data item's clock settings.  This method simply calls {@link CatalogItem#zoomTo} and
 * {@link CatalogItem#useClock}.  Because the zoom and clock update may happen asynchronously (for example, if the
 ...
 * * Will not necessarily be called immediately when {@link CatalogItem#isEnabled} is set to true; it will be deferred
 * until
 *   {@link CatalogItem#isLoading} is false.
 * * Should NOT also show the data item on the globe/map (see {@link CatalogItem#_show}), so in some cases it may not
 * do
 *   anything at all.
 * * Calls {@link CatalogItem#_enableInCesium} or {@link CatalogItem#_enableInLeaflet} in the base-class
 * implementation,
 *   depending on which viewer is active.  Derived classes that have identical enable logic for both viewers may
 * override this method instead of the viewer-specific ones.
 * @protected
 */
 ...
 * * Will not be called if {@link CatalogItem#_enable} was not called (for example, because the previous call was
 * deferred while the data item loaded, and the user disabled the data item before the load completed).
 * * Will only be called after {@link CatalogItem#_hide} when a shown data item is disabled.
 * * Calls {@link CatalogItem#_disableInCesium} or {@link CatalogItem#_disableInLeaflet} in the base-class
 * implementation, depending on which viewer is active.  Derived classes that have identical disable logic for both
 * viewers may override this method instead of the viewer-specific ones.
 * @protected
 */
 ...
 * * Will not necessarily be called immediately when {@link CatalogItem#isShown} is set to true; it will be deferred
 * until
 *   {@link CatalogItem#isLoading} is false.
