...
var WebMapServiceCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the formats in which to try WMS GetFeatureInfo requests.  If this property is undefined, the
     * `WebMapServiceImageryProvider` defaults are used.  This property is observable.
     * @type {GetFeatureInfoFormat[]}
     */
    this.getFeatureInfoFormats = undefined;

    /**
     * Gets or sets a value indicating whether a time dimension, if it exists in GetCapabilities, should be used to
     * populate the {@link ImageryLayerCatalogItem#intervals}.  If the {@link ImageryLayerCatalogItem#intervals}
     * property is set explicitly on this catalog item, the value of this property is ignored.
     * @type {Boolean}
     * @default true
     */
    ...
    /**
     * Gets or sets the denominator of the largest scale (smallest denominator) for which tiles should be requested.
     ...
     * specified by this property, will be used and will simply get blurier as the user zooms in closer.
     * @type {Number}
     */
    ...
    knockout.track(this, [
        '_metadataUrl', 'legendUrls', '_rectangle', '_rectangleFromMetadata', '_intervalsFromMetadata',
        'layers', 'parameters', 'getFeatureInfoFormats',
        'tilingScheme', 'populateIntervalsFromTimeDimension', 'minScaleDenominator']);

    // metadataUrl and legendUrls are derived from url if not explicitly specified.
    ...
    overrideProperty(this, 'legendUrls', {
        get: function() {
            if (defined(this._legendUrls) &amp;&amp; this._legendUrls.length &amp;&amp; this._legendUrls[0].isValid()) {
    ...
    // The dataUrl must be explicitly specified.  Don't try to use `url` as the the dataUrl, because it won't work for
    // a WMS URL.
...
defineProperties(WebMapServiceCatalogItem.prototype, {
    ...
    /**
     * Gets a value indicating whether this {@link ImageryLayerCatalogItem} supports the {@link
        * ImageryLayerCatalogItem#intervals} property for configuring time-dynamic imagery.
     * @type {Boolean}
     */
    supportsIntervals: {
        get: function() {
 ...
 * @param {Object|XMLDocument} capabilities The capabilities document.  This may be a JSON object or an XML document.
 *     If it is a JSON object, each layer is expected to have a `_parent` property with a reference to its parent
 *     layer.
 * @param {Boolean} [overwrite=false] True to overwrite existing property values with data from the capabilities; false
...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    if (thisLayer.Style) {
        ...
        if (style.LegendURL) {
            ...
            updateValue(this, overwrite, '_legendUrls', [legend]);
        }
    }
...
function addDurationFromString(start, durationString, wmsItem) {
    ...
    if (matches) {
        ...
        if (matches[2]) {
            stop.setDate(stop.getDate() + (+matches[2]));
        }
        if (matches[4]) {
            stop.setHours(stop.getHours() + (+matches[4]));
        }
        if (matches[6]) {
            stop.setMinutes(stop.getMinutes() + (+matches[6]));
        }
        if (matches[8]) {
            stop.setSeconds(stop.getSeconds() + (+matches[8]));
        }
    } else {
